...
│   └── fix_staging.py            # __time → timestamp normaliser (workaround)
├── pinot-overrides/
...
Three rough edges surfaced in the hybrid path. The first two are
addressed in this repo by [an accompanying fix to backfill_runner.py];
the rest are worked around in `run.sh`/`fix_staging.py`. Each is worth a
PR.
...
| 3 | `dpm backfill-batch` exports Druid SQL `SELECT *` rows whose time column is named `__time` and typed as an ISO 8601 string. The Pinot schema's time column is whatever the user named it (here `timestamp`) typed as `LONG/MILLISECONDS:EPOCH`. Pinot silently drops `__time` and inserts default zeros, collapsing the OFFLINE segment's `[startTime, endTime]` to a single ms and breaking hybrid time-boundary routing. | `data/fix_staging.py` renames `__time` → `timestamp` and converts ISO → millis; `run.sh` step 12 runs it before re-ingesting. |
